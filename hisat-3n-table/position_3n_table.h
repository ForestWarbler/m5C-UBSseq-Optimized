...
public:
    ...
    // Single-argument constructor for simplified interface (used in hisat_3n_table.cpp)
    explicit Positions(string inputRefFileName) : Positions(inputRefFileName, 1, false, false) {
    }
    ...
    /**
     * load new chromosome with meetNext parameter (for single-thread mode)
     */
    void loadNewChromosome(string targetChromosome, int& meetNext) {
        meetNext = 0; // initialize
        refFile.clear();
        // find the start position in file based on chromosome name.
        streampos startPos = chromosomePos.getChromosomePosInRefFile(targetChromosome);
        chromosome = targetChromosome;
        ...
        while (refFile.good()) {
            getline(refFile, line);
            if (line.front() == '>') { // this line is chromosome name
                meetNext = 1; // met next chromosome
                return;
            } else {
                if (line.empty()) { continue; }
                // change all base to upper case
    ...
    /**
     * load more Position with meetNext parameter (for single-thread mode)
     */
    ...
    /**
     * Synchronous append for single-thread mode (used in hisat_3n_table.cpp)
     * Directly process SAM line without queueing
     */
    void appendSync(const string& line) {
        if (refPositions.empty()) {
            return; // no reference positions loaded
        }
        
        Alignment newAlignment;
        string* linePtr = new string(line);
        newAlignment.parse(linePtr);  // 移除 & 操作符
        appendPositions(newAlignment);
        delete linePtr;
    }

    /**
     * Start output processing (used in hisat_3n_table.cpp)
     * If flushAll is true, output all remaining positions
     * Otherwise, output positions that are outside the loading window
     */
    void startOutput(bool flushAll = false) {
        if (flushAll) {
            moveAllToOutput();
        } else {
            moveBlockToOutput();
